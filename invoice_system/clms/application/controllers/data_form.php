...
class Data_form extends CI_Controller
{
	 ...
	 *
	 *  form process for the General Form.
	 */
	...
	public function edit_form()
	{
		...
		$this->form_validation->set_rules('name', 'Nama','trim|required|xss_clean');
		$this->form_validation->set_rules('ic_num', 'No Kad Pengenalan dengan betul','trim|required|xss_clean');
		$this->form_validation->set_rules('address', 'Alamat','trim|required|xss_clean');
		$this->form_validation->set_rules('phone_num', 'No. Telefon','trim|required|xss_clean');
		$this->form_validation->set_rules('email', 'Email','trim|required|xss_clean');
		if ($this->form_validation->run() == FALSE)
		{
			...
			$data = array(
						  'title' => "Edit Data",
						  'upload_error' => '',
						  'update_error' => 'Data gagal dikemaskini',
		...
		else
		{
			...
			$data = array(
				'title' => "Edit Data",
				'get_data' => $this->db_action->get_field_by_id(),
				'update_success' => 'Data berjaya dikemaskini'
				);
			$this->load->view('template/header',$data);
			$this->load->view('template/navbar',$data);
			$this->load->view('edit_form',$data);
	...
	public function action_form()
	{
		...
		switch($post)
		{
			...
			case isset($post['delete']):
				...
				redirect('database/index','refresh');
				break;
